...
export const findAndExecuteInDom = (args: FinderArgs[] | undefined) => {
  ...
  args.forEach((arg, i) => {
    ...
    let doc = arg.htmlIframe?.length
      ? (window as any).frames[arg.htmlIframe].document.querySelectorAll(
          arg.htmlElement
        )
...
//   "https://s-at00-163.meusburger-norm.com/s-at00-162_cwpdb1_cronet/!MAServ.MAServ_Main";
...
export function renderClockInHomeOffice(
  clockIn: ClockInTypes,
  username: string,
  password?: string
) {
      ...
      [
        {
          ...
          htmlIframe: "Menue",
          textContent:
            clockIn === "clockIn" ? "Kommt mit Grund" : "Geht mit Grund",
          textPlacement: "textContent",
        },
      ],
      [
...
export function renderClockInHomeOfficeT(
  clockIn: ClockInTypes,
  username: string,
  password?: string
) {
